...
const getKeyDisplayLabel = (keyCode: string) => {
...
// Map key codes to the actual character they produce
...
export const getCharFromKeyCode = (keyCode: string): string | null => {
  if (KEY_CHAR_MAP[keyCode]) return KEY_CHAR_MAP[keyCode];
...
const KeyboardProvider = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      setPressed(keyCode);
      // Don't call onKeyPress here — physical keyboard input is handled
      // natively by the input element. onKeyPress is only for on-screen clicks.
    };
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={cn(
          "mx-auto w-fit [zoom:0.65] sm:[zoom:0.85] md:[zoom:1] lg:[zoom:1.2] xl:[zoom:1.4]",
          className
        )}
      >
  ...
  return (
    <div className="h-full w-fit rounded-xl bg-neutral-200 p-1 shadow-sm ring-1 shadow-black/5 ring-black/5">
      ...
      <Row>
        ...
        <Key keyCode="Space" containerClassName="flex-1" className="w-full" />
        ...
        {/* Arrow Keys */}
        <div className="flex h-6 flex-1 items-center justify-end rounded-[4px] p-[0.5px]">
...
const Key = ({
  ...
  const { playSoundDown, playSoundUp, pressedKeys, setPressed, setReleased, onKeyPress } =
    useKeyboardSound();
